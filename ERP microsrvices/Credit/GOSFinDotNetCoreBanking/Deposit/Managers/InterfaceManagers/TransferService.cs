        ...
        public async Task<bool> UploadTransferSetupAsync(List<byte[]> record)
        {
            try
            {
                ...
                if (uploadedRecord.Count > 0)
                {
                    ...
                    {
                        var compid = comp.companyStructures.FirstOrDefault(e => e.name.ToLower() == item.CompanyName.ToLower())?.companyStructureId;
                        var prod = _dataContext.deposit_accountsetup.FirstOrDefault(e => e.AccountName.ToLower() == item.ProductName.ToLower())?.DepositAccountId;
                        var type = _dataContext.deposit_accountype.FirstOrDefault(e => e.Name.ToLower() == item.AccountTypeName.ToLower())?.AccountTypeId; 
