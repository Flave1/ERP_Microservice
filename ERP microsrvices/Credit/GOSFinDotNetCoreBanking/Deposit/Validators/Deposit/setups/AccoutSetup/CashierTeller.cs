...
namespace Deposit.Validators.AccoutSetup
{
    public class AddUpdateCashierTellerSetupObjVal : AbstractValidator<AddUpdateCashierTellerSetupObj>
    {
        ...
        public AddUpdateCashierTellerSetupObjVal(DataContext dataContext)
        {
            ...
            RuleFor(r => r).MustAsync(NoDuplicateAsync).WithMessage("Staff already assigned to a product");
        }
        ...
        private async Task<bool> NoDuplicateAsync(AddUpdateCashierTellerSetupObj request, CancellationToken cancellationToken)
        {
            ...
            {
                var item = _dataContext.deposit_cashiertellersetup.FirstOrDefault(e => e.Employee_ID == request.Employee_ID  && e.DepositCashierTellerSetupId != request.DepositCashierTellerSetupId && e.Deleted == false);
                if (item != null)
                {
            ...
            if (_dataContext.deposit_cashiertellersetup.Count(e => e.Employee_ID  == request.Employee_ID  && e.Deleted == false) >= 1)
            {
                return await Task.Run(() => false);
            }
            return await Task.Run(() => true);
