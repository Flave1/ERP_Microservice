...
namespace Deposit.Validators.AccoutSetup
{
    public class AddUpdateWithdrawalSetupObjVal : AbstractValidator<AddUpdateWithdrawalSetupObj>
    {
        ...
        private async Task<bool> NoDuplicateAsync(AddUpdateWithdrawalSetupObj request, CancellationToken cancellationToken)
        {
            ...
            {
                var item = _dataContext.deposit_withdrawalsetup.FirstOrDefault(e => e.Product == request.Product && e.WithdrawalSetupId != request.WithdrawalSetupId && e.Deleted == false);
                if (item != null)
                {
            ...
            if (_dataContext.deposit_withdrawalsetup.Count(e => e.Product == request.Product && e.WithdrawalSetupId == request.WithdrawalSetupId && e.Deleted == false) >= 1)
            {
                return await Task.Run(() => false);
            }
            return await Task.Run(() => true);
