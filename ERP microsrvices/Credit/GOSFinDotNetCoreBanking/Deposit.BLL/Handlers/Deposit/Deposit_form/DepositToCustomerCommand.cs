...
namespace Deposit.Handlers.Deposit.Deposit_form
{
    public class Deposit_to_customerCommandHandler : IRequestHandler<Deposit_to_customer, Account_response>
    {
        ...
        private Account_response Validate_transaction(deposit_form request)
        {
            ...
            var customer_account_details = _dataContext.deposit_customer_accountdetails.FirstOrDefault(e => e.AccountNumber.ToLower() == request.Account_number.ToLower());
        ...
        { 
            var customer = _dataContext.deposit_customer_accountdetails.FirstOrDefault(e => e.AccountNumber.ToLower() == deposit.Account_number.ToLower());
            if(customer != null)
            {
